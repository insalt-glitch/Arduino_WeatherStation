...
//    if (result == OK) Serial.println("K");// ("ok");
    if (result == ERR) Serial.println("E");// ("error");
  }
}

/*
 * We have a maximum total of 61 Bytes to write to (sizeOf(data[]))
 * Takes in a pointer to the data and the length.
 * Please make sure not to sent RF-AT-Commands. ATxxx OR "+++"
 */
